      ...
      <section className="bg-primary py-12 sm:py-16">
        <div className="container-main text-center px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <span className="text-gold font-bold text-sm uppercase tracking-wider">Get In Touch</span>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <Input id="cName" value={name} onChange={(e) => setName(e.target.value)} placeholder="John Doe" />
                </div>
                <div>
                  <Label htmlFor="cEmail">Email Address</Label>
                  <Input id="cEmail" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="john@example.com" />
                </div>
                <div>
                  <Label htmlFor="cMessage">Message</Label>
                  <Textarea id="cMessage" value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Your message..." rows={5} />
                </div>
                <Button type="submit" className="bg-secondary text-secondary-foreground hover:opacity-90 font-bold">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <div className="space-y-6 mb-8">
                {[
                  { icon: MapPin, label: "Address", value: "Kiambu County Government Offices\nKiambu Town, Kenya" },
                  { icon: Mail, label: "Email", value: "info@kiambuchess.co.ke" },
                  { icon: Phone, label: "Phone", value: "0728008475" },
                ].map((item) => (
              ...
              <div className="flex gap-3">
                {[
                  { label: "Facebook", href: "https://www.facebook.com/kiambucountychessassociation/", path: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" },
                  ...
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                    className="w-10 h-10 rounded-full bg-secondary/10 hover:bg-secondary hover:text-secondary-foreground flex items-center justify-center transition-all duration-200 text-secondary"
                    aria-label={s.label}>
